...
class Ship:
    ...
    def shoot(self):
        ...
        if started:
            ...
            missile_pos = [self.pos[0] + self.radius *
    ...
    if started:
        if len(rock_group) < MAX_ROCK:
            if dist(rock_pos, my_ship.get_position()) > asteroid_info.get_radius() + my_ship.get_radius() + 10:
                rock_group.add(Sprite(rock_pos, rock_vel, 0,
                               rock_avel, asteroid_image, asteroid_info))
...
def group_collide(group, other):
    ...
    for sprite in group:
        if sprite.collide(other):
            explosion_group.add(
                Sprite(sprite.pos, [0, 0], 0, 0, explosion_image, explosion_info))
            explosion_sound.rewind()
